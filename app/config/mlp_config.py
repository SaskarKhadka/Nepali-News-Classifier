...
def get_parameters():
    with codecs.open(settings.MLP_PARAMETERS_PATH, encoding='utf-8') as f:
        data = json.load(f)
    return data


def get_tokenizer():
    with codecs.open(settings.MLP_TOKENIZER_PATH, encoding='utf-8') as f:
        data = json.load(f)
        tokenizer = tf.keras.preprocessing.text.tokenizer_from_json(data)
    return tokenizer

def get_model():
    return keras.models.load_model(settings.MLP_MODEL_PATH)

def get_label_encoder():
    with open(settings.MLP_LABEL_ENCODER, 'rb') as f:
        le = pickle.load(f)
    return le
